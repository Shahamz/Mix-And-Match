"""
PixelDiT text-to-image transformer, copied from NVlabs/PixelDiT (commit 41f7300) and adapted for Mix-And-Match.
...
tracking, the unused text-mask and precomputed-condition paths, zero-probability dropouts, the PiT post-modulation
branch, and the switches the released checkpoint always turns on (text RoPE, per-pixel absolute positions). Every
changed spot is marked "Mix-And-Match:".
...
Mix-And-Match changes: the forward takes a stack of S images (S = 1 before the split, k after) and runs them as ONE
token sequence. All S images reuse the same RoPE positions, so every tile sees itself at its crop's place in the
full image. Attention goes through attention.attention, masked by a TileAttentionLayout; after the split the
...
from macros import drop_page_cache
...
ARCHITECTURE_KEYS = (
    ...
    "pixel_num_groups", "patch_depth", "pixel_depth", "txt_embed_dim", "txt_max_length", "text_rope_theta",
)
...
def average_over_tiles(tokens, layout):
    """
    Mix-And-Match: averages the tile tokens over the k tiles. Image s holds tile s of every crop, so the result holds,
...
class PixelTokenEmbedder(nn.Module):
    """Embeds every pixel (plus an absolute sine-cosine position embedding) and groups the pixels by patch."""
    ...
    def _fetch_pixel_pos_image(self, height, width, device, dtype):
        ...
        Mix-And-Match: the original's separate square-image branch computed the same values and was removed.
...
class RotaryAttention(nn.Module):
    ...
    def forward(self, x, pos, layout=None):
        ...
        # Mix-And-Match: append the crop-average keys/values and use the pixel-stage tile mask.
        if layout is not None and layout.append_crop_averages:
            k = torch.cat([k, average_over_tiles(k, layout)], dim=1)
            v = torch.cat([v, average_over_tiles(v, layout)], dim=1)
...
class PiTBlock(nn.Module):
    ...
    def __init__(self, pixel_hidden_size, patch_hidden_size, patch_size, num_heads, mlp_ratio=4.0,
                 attn_hidden_size=None, attn_num_heads=None):
        """
        Args:
    ...
    def _fetch_pos(self, height, width, device):
        ...
        key = (height, width)
        if key not in self._pos_cache:
            self._pos_cache[key] = precompute_freqs_cis_2d(self.attn_dim // self.num_heads, height, width)
    ...
    def forward(self, x, s_cond, image_height, image_width, patch_size, num_images=1, layout=None):
        ...
        Args:
            ...
            patch_size: patch side in pixels.
            num_images: Mix-And-Match: number of images (k) joined into one sequence.
            layout: Mix-And-Match: TileAttentionLayout, or None for full attention.
        ...
        # Mix-And-Match: all images form one sequence, with the background crop's patches kept once (attention.py).
...
class MMDiTJointAttention(nn.Module):
    ...
    def forward(self, x, y, pos_img, pos_txt, layout=None):
        """
        Args:
            ...
            pos_txt: [prompts * prompt length, head dim // 2] text RoPE frequencies.
            layout: Mix-And-Match: TileAttentionLayout, or None for full attention (before the split).
        ...
        # Mix-And-Match: at the saliency step, patch-to-prompt attention scores are measured on the side.
        if layout is not None and layout.saliency_probe is not None:
            layout.saliency_probe.record(qx, ky)

        # Token order is [text, image]; Mix-And-Match: crop-average keys/values go last, and the tile mask is used.
        q_joint = torch.cat([qy, qx], dim=1)
        k_joint = torch.cat([ky, kx], dim=1)
        v_joint = torch.cat([vy, vx], dim=1)
        if layout is not None and layout.append_crop_averages:
            k_joint = torch.cat([k_joint, average_over_tiles(kx, layout)], dim=1)
            v_joint = torch.cat([v_joint, average_over_tiles(vx, layout)], dim=1)
...
class MMDiTBlockT2I(nn.Module):
    ...
    def forward(self, x, y, c, pos_img, pos_txt, layout=None):
        """
        Args:
...
class PixDiT_T2I(nn.Module):
    ...
    def __init__(self, in_channels=3, num_groups=16, hidden_size=1152, pixel_hidden_size=64,
                 ...
                 txt_embed_dim=4096, txt_max_length=1024, text_rope_theta=10000.0):
        """
        Args:
            in_channels: image channels (3).
            ...
            text_rope_theta: base of the text tokens' 1D RoPE.
        """
        super().__init__()
    ...
    @property
    def device(self):
        """Mix-And-Match: device of the weights (DiffusionPipeline reads it from its components)."""
    ...
    def forward(self, images, timestep, text_embeds, layout=None):
        """
        Mix-And-Match: runs a stack of S images and P prompts as ONE sequence per batch element.
        ...
        # Mix-And-Match: patchify every image and join them into one sequence; all images reuse the same positions,
        ...
        if layout is None:
            ...
            # Mix-And-Match: the background crop's patches are kept once, from image 0 (attention.py).
        ...
        # Mix-And-Match: every prompt gets its own learned positions and text RoPE, as if it were alone.
        ...
        pos_txt = self.fetch_pos_text(text_length, images.device).repeat(P, 1)
        ...
        # Mix-And-Match: every image gets its tile patches back and a copy of the single background region.
...
def load_pixeldit(device, dtype, weights_dir=None):
    """
    Builds PixDiT_T2I from the released checkpoint.

    By default the checkpoint and its config are downloaded once into the Hugging Face cache; with weights_dir
    they are read from that folder instead.
    ...
    Args:
        ...
        weights_dir: Path of a folder holding config.json and pixeldit_t2i_v1.pth, or None to download them.

    Returns:
        (model in eval mode, repository config dict; its "scheduler" entry holds the flow shift).

    Raises:
        FileNotFoundError: if weights_dir lacks one of the files.
    ...
    for filename in (MODEL_CONFIG_FILENAME, CHECKPOINT_FILENAME):
        if weights_dir is None:
            paths[filename] = Path(hf_hub_download(CHECKPOINT_REPO_ID, filename))
        else:
            paths[filename] = weights_dir / filename
            if not paths[filename].is_file():
                raise FileNotFoundError(f"{paths[filename]} does not exist (expected {CHECKPOINT_REPO_ID}/{filename})")
    model_config = json.loads(paths[MODEL_CONFIG_FILENAME].read_text())
    with torch.device("meta"):
        model = PixDiT_T2I(**{key: model_config[key] for key in ARCHITECTURE_KEYS})

    checkpoint_path = paths[CHECKPOINT_FILENAME]
